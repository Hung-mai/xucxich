...
public class SnakeBlock_Manager: Minigame_Manager {
    public static SnakeBlock_Manager ins;
    ...
    public override void InitGame() {
        ...
        for (int i = 1; i < maxCharacter; i++) {
            ...
            listChars[i].AIBot.pos_TargetMove = listCheckpoint[listChars[i].idCheckpoint + 1].transform.position;
            ...
            listChars[i].speed = speedChar*1.1f;
    ...
    private void OnDrawGizmos() {
        ...
        for (int j = 0; j < listCheckpoint.Count; j++) {
            Gizmos.color = Color.blue;
            Gizmos.DrawWireSphere(new Vector3(listCheckpoint [j].transform.position.x, listCheckpoint [j].transform.position.y, listCheckpoint [j].transform.position.z), 20f);
        }
    }
}
